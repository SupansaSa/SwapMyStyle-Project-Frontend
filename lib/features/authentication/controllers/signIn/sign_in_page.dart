...
import 'package:myapp/features/authentication/controllers/forgot_password/forgot_password_page.dart';
import 'package:myapp/features/authentication/controllers/signUp/sign_up_page.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final response = await http.post(
    Uri.parse('http://192.168.31.218:3000/login'),
  ...
  if (response.statusCode == 200) {
    if (jsonResponse['success']) {
      ...
      if (user != null && user['id'] != null) {
        ...
        _showErrorDialog('เกิดข้อผิดพลาด', 'ไม่พบข้อมูลผู้ใช้ในคำตอบจากเซิร์ฟเวอร์');
      }
    } else {
      _showErrorDialog('เกิดข้อผิดพลาด', jsonResponse['message']);
    }
  } else {
    _showErrorDialog('เกิดข้อผิดพลาด', jsonResponse['message'] ?? 'เกิดข้อผิดพลาดในการเข้าสู่ระบบ');
  }
}
                        ...
                        : CustomButton(
                            ...
                            onPressed: (BuildContext context) {
                              if (_emailController.text.isEmpty || _passwordController.text.isEmpty) {
                                _showErrorDialog('เกิดข้อผิดพลาด', 'กรุณากรอกอีเมลและรหัสผ่าน');
                                return;
                              }
                              _login();
                            },
